                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center justify-center w-8 h-8 text-sm font-bold text-white bg-[#DE6262] border border-[#DE6262] cursor-default rounded-full mx-1 shadow-sm">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center justify-center w-8 h-8 text-sm font-medium text-[#3A384C] bg-white border border-gray-300 hover:bg-gray-100 hover:text-[#DE6262] transition ease-in-out duration-150 rounded-full mx-1 shadow-sm" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
